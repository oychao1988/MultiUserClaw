...
def build_openclaw_image():
    """构建 openclaw-user 基础镜像（用户容器使用）。"""
    log("构建 openclaw-user:2026.4.10 基础镜像...")
    run("docker build --no-cache -f openclaw/Dockerfile.bridge -t openclaw-user:2026.4.10 openclaw/")
    success("openclaw-user:2026.4.10 构建完成")


def build_openclaw_image_fast():
    """使用缓存构建 openclaw-user 基础镜像（用户容器使用）。"""
    log("构建 openclaw-user:2026.4.10 基础镜像（使用缓存）...")
    run("docker build -f openclaw/Dockerfile.bridge -t openclaw-user:2026.4.10 openclaw/")
    success("openclaw-user:2026.4.10 构建完成")
...
def main():
    ...
    if not args.skip_base:
        ...
        tasks = {
            "openclaw-user:2026.4.10": "docker build --no-cache -f openclaw/Dockerfile.bridge -t openclaw-user:2026.4.10 openclaw/",

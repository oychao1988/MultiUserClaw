...
4，任务每次结束之后可不可以有一个通知
5，上传的文件名字太长就延长掉了删除按钮按钮的位置，就无法删除
6，模型回复太多了，比如写文献的时候就会显示被截断，这个机制需不需要修正
7，思考和做了什么的过程能展示吗或者以折叠的方式，当前只展示结果

1、增加一个简单用户界面，没有管理配置功能，只有对话功能。
这个界面调用时：
①增加Agent标识参数，明确当前使用的Agent。
②增加用户唯一标识参数（如工号、手机号、用户ID），避免多用户会话串聊、相互干扰。

...
- 容器的端口信息显示
- 可以直接下载目录
- 分离代码：`frontend/src/components/Brand.tsx` 单独设置 Logo 和简介信息
- 离线部署脚本 `offline_deploy.py`
- 对话框中的斜杠命令添加
- 显示会话时，显示与哪个 Agent 的对话
- 修复回答需要刷新才能显示完整的多轮回答的 Bug
- 升级 openclaw 为 0324 版本和会话重命名
- OpenBrowser 浏览器支持，Agent 状态显示，Minmax 加到环境变量
- 微信二维码显示优化（更好的字体和间距）
